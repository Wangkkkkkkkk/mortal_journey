...
1. <thinking>思考分析</thinking>
   ...
   - 开局配置：根据主角出身、境界和剧情描述，构思开局法宝（2-3个）、功法（攻击+辅助各一）、储物袋（灵石/丹药/杂物），须与剧情契合且不过强不过弱。
...
6. 示例：<mj_equip_body> [{"type":"法宝","name":"青锋剑","intro":"外门制式灵剑，刃口隐隐泛着灵光"},{"type":"法宝","name":"玄铁护甲","intro":"以玄铁炼制的护身甲片，轻便坚韧"}] </mj_equip_body>
...
5. intro 规则：
  5.1 只描述功法的来历、传承渊源、外观形态、修炼时的意境感受。
  5.2 禁止在 intro 中使用"体修""法修""剑修""毒修""药修""魔修""通用""灵力""劲力""体魄"等体系或属性分类名词。
  5.3 不得描述功法的具体功能效果，应采用修仙世界的玄学化描述风格。
  5.4 示例："据传源自上古力修一脉，修炼时周身气血如山岳崩裂"。
6. system（体系）字段：只能从以下七种中选择一个：通用、剑修、体修、法修、毒修、药修、魔修。体系须与功法名称和描述契合，参考如下：
   - 通用：万法归宗，不拘一格，物理法术兼修（如长春功、聚灵诀、混元功）。
   - 剑修：御剑杀伐，以剑为道，飞剑体系为主（如御剑法、青莲剑歌、斩仙剑诀）。
   - 体修：淬炼肉身，以力破万法，高血高防普攻为主（如崩山诀、金刚不坏功、龙象诀）。
   - 法修：五行术法，驾驭天地灵气，法术伤害与控制为主（如烈火术、冰魄法、天雷法）。
   - 毒修：以毒入道，炼蛊施毒，叠毒爆发为主（如万毒诀、化骨术、噬魂蛊法）。
   - 药修：炼丹治伤，以药入道，恢复与辅助为主（如回春术、灵愈法、造化丹诀）。
   - 魔修：以血祭法，嗜血狠辣，祭血增伤与削弱为主（如噬血魔功、天魔诀、炼魂法）。
7. role（定位）字段：
  7.1 只能从"攻击"和"辅助"中选择一个。
  7.2 攻击类功法（如崩山诀、烈火术）必须选择"攻击"，辅助类功法（如聚灵诀、轻身术）选择"辅助"。功法名称明显是攻击手段时必须选"攻击"。
  7.3 role 决定系统分配的功法效果类型：
    - 攻击 → 主动效果（造成伤害+附加效果）。
    - 辅助 → 被动效果（增益/减益/触发）。
8. 功法命名硬约束：
  8.1 功法名称的最后一个字必须是"功""诀""术""法"之一，禁止使用其他字结尾。
  8.2 正确示例：长春功、聚灵诀、烈火术、御剑法。
  8.3 错误示例：碎石掌、混元劲、金钟罩。
9. 示例：<mj_magic_body> [{"type":"功法","name":"崩山诀","intro":"据传源自上古力修一脉，修炼时周身气血如山岳崩裂","bonus":"劲力","system":"体修","role":"攻击"},{"type":"功法","name":"聚灵诀","intro":"宗门世代相传的基础功法，修炼时丹田如清泉汇聚","bonus":"灵力","system":"法修","role":"辅助"}] </mj_magic_body>

[储物袋开局配置规则]
1. 主角储物袋开局配置：可以生成灵石、丹药、材料、杂物等。
  1.1 类型为"材料"时必须输出 category 字段，只能从药材、毒物、食材、器材四种中选择一个（药材用于炼丹，毒物用于制毒，食材用于烹饪，器材用于炼器），须与材料名称契合。
  2. 灵石生成规则：
...
3. 好感度初始化：默认落在 -19~19。
4. NPC的法宝结构：
  4.1 法宝须含 type（法宝）、name、intro。不需要 function 字段。
  4.2 功法结构与主角相同：功法须含 type（功法）、name、intro、bonus、system、role。不需要 function 字段。
  4.3 role 为"攻击"或"辅助"，攻击类功法必须选"攻击"。
