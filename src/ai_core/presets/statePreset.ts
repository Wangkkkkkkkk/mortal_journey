...
[消耗品效果规则（丹药/符箓/阵法）]
  ...
  3. 丹药/符箓/阵法必须输出 grade（判定依据见「储物袋物品添加规则」第6条）。
...
5. 物品品阶和境界对应关系（境界参照下限）：练气对应下品，筑基对应中品，结丹对应上品，元婴对应极品，化神对应仙品，神品为超越化神的至高品阶。
6. 物品品阶（grade）必须由你判定并输出，且每件添加的物品都必须携带 grade，不得省略。判定依据为「主角当前境界的参照下限」与「物品描述分量」共同决定：
   - ① 境界参照下限：按第5条映射，练气=下品、筑基=中品、结丹=上品、元婴=极品、化神=仙品。这是判定时的基准档位。
   - ② 描述分量（在基准档位之上决定具体档位）：
     · 凡俗寻常、批量制式、随手可得、路边拾取的物品 → 取境界参照下限。
     · 精品、有名有姓、坊市可购的较好之物、同境修士的日常用物 → 参照下限上 1 档。
     · 重宝、古修遗迹遗物、高人前辈的赠物或遗泽、剧情明确强调珍稀/机缘/稀有的物品 → 取更高档（可至仙品、神品）。
   - 系统只做合法性校验，不再随机生成品阶；你输出的合法品阶会被原样保留（即使低于参照下限，也信任你的判断）。
...
8. 物品信息：
  ...
  8.4 品阶 grade：必须填写，只能是下品、中品、上品、极品、仙品、神品中的一个。判定依据见第6条（境界参照下限 + 描述分量），系统只做合法性校验、不随机生成。
...
7. NPC的功法结构：
  ...
  7.2 功法命名硬约束：功法名称的最后一个字必须是"功""诀""术""法"之一，禁止使用其他字结尾。
   7.3 功法必须输出 grade（品阶由你判定并输出，以该 NPC 自身境界为参照下限、结合物品描述分量确定，同「储物袋物品添加规则」第6条；系统只做合法性校验、不随机生成）。主动/被动由所选 kind 自动决定。
8. NPC储物袋中的消耗品（丹药/符箓/阵法）须含 grade 和 effects（[{kind}]），grade 必须填写，以该 NPC 自身境界为参照下限确定。
9. NPC生成需要包含的信息：
