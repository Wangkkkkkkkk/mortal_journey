...
1. <thinking>思考分析</thinking>
   思考内容：
   - 场景地点：开局剧情中主角当前身处何处？按四级地点规则（大区域-国家-区域-具体地点）确定开局主场景。第4级（具体地点）必须取自剧情正文中主角当前所处场景（如执事堂、洞府、坊市档口），禁止照抄出生地的第4级；前三级的「大区域-国家-区域」在剧情未跨域时与出生地保持一致。
   ...
   - 人物盘点：开局剧情中出现了哪些NPC？逐一确认其身份、境界、灵根、性别、年龄、性格、爱好、恐惧、当前目标、长远目标；与主角是何关系；对主角的开局处境有何影响。
    - 开局配置：严格只从剧情正文提取主角的法宝、功法、储物袋物品（灵石/丹药/符箓/阵法/杂物）。剧情正文未提及的一律不得输出（法宝/功法输出空数组，储物袋不输出该物品/灵石）。禁止按境界、出身、修仙常识自行补配任何物品。
...
2. 标签内格式为四级地点，用"-"分隔：大区域-国家-区域-具体地点。例如：天南-越国-黄枫谷-执事堂。须非空。
3. 四级含义：第一级为大区域（如天南、乱星海、大晋），第二级为国家或势力范围（如越国、九国盟），第三级为区域或宗门（如黄枫谷、七玄门），第四级为具体地点（如执事堂、外门、坊市、洞府）。
4. 第4级（具体地点）必须是剧情正文中主角此刻所处场所，以剧情描写为准，禁止直接照抄出生地的第4级（如出身在黄枫谷外门，但剧情正文写主角在执事堂，则第4级应写执事堂）。
5. 若剧情正文未明示具体场所，则回退到出生地的第4级，保证四级地点完整非空。

[法宝开局配置规则]
1. 严格从开局剧情正文中提取主角的法宝：法宝的名称和效果必须能在剧情正文中找到对应描写。剧情正文未提及任何法宝时，必须输出空数组 []，禁止自行编造或按境界补配。
...
[功法开局配置规则]
1. 严格从开局剧情正文中提取主角的功法：功法的名称和效果必须能在剧情正文中找到对应描写。剧情正文未提及任何功法时，必须输出空数组 []，禁止自行编造或按境界补配。数量由剧情决定，可能为0（空数组）。
...
[储物袋开局配置规则]
1. 严格从开局剧情正文中提取主角储物袋物品：灵石、丹药、符箓、阵法、材料、杂物等，名称与数量必须与剧情正文一致。剧情正文未提及的物品（含灵石）一律不得输出。
2. 灵石规则：灵石不区分品阶（无上品灵石、中品灵石之分），统一称为"灵石"。数量只能取自剧情正文中实际提到的灵石数量；剧情正文未提及灵石则不含灵石，禁止按主角境界自行估算灵石数量。
3. 储物袋物品信息：所有物品（除灵石外）必须输出 grade，只能是下品、中品、上品、极品、仙品、神品中的一个，由你根据剧情和主角境界决定，系统尊重你的输出。丹药/符箓/阵法还需包含 effects 字段。
4. 输出格式：<mj_storage_body> … </mj_storage_body>，内为 JSON 数组。
5. 示例（仅演示格式，物品与数量以剧情正文为准）：<mj_storage_body> [{"type":"灵石","name":"灵石","count":10},{"type":"丹药","name":"辟谷丹","intro":"碧绿丹丸，隐有草木清香","grade":"下品","effects":[{"kind":"healMp"}],"count":2},{"type":"杂物","name":"宗门令牌","intro":"外门弟子通行木牌","grade":"下品","count":1}] </mj_storage_body>
