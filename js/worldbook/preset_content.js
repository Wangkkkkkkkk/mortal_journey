    ...
    presets: [
      ...
      },
      {
        ...
        systemPrompt: [
          ...
          "叙事：第三人称；战斗时写清距离、招式与法宝/法术名目、场面张力，避免一口糊掉胜负；可用文学化描写表现狼狈或吃紧，但不要写出血量、法力的具体损耗、剩余或加减结算。",
          "规则：尊重玩家输入；不擅自宣告玩家死亡，除非玩家明确要求。",
          "数值边界：修为与境界进度仅由玩家在游戏内机制提升；禁止在文中写「修为 +N」、当前修为分数或【系统提示】式修为播报，禁止因战斗/修炼替玩家改修为或境界。血量、法力完全由游戏面板管理，文中禁止数值化结算这两项。",
          "需要随机性时可轻描淡写带过判定逻辑，不必输出 JSON。",
